...
class KernelTest(unittest.TestCase):
    ...
    def test_rho_kernel(self):
        ...
        cov = kernel(rho=[0.5])
        assert(np.allclose(cov[0], np.array([9/16., 3/16, 3/16, 1/16])))
    ...
    def test_rho_kernel_grad(self):
        ...
        assert(np.allclose(grads[0][0], np.array([ 0.375, 0.125, -0.375,  -0.125]) / 4))
        assert(np.allclose(grads[1][0], np.array([ 0.375, -0.375,  0.125, -0.125]) / 4))
    ...
    def test_full_kernel_alignment_comparison(self):
        np.random.seed(0)
        l, a = 4, 4
        alleles = ['A', 'C', 'G', 'T']
    ...
    def test_full_kernel_alignment_rho(self):
        X = np.array(['AA', 'AB', 'BA', 'BB'])
        alleles = ['A', 'B']
    ...
    def test_full_kernel_alignment_rho_comparison(self):
        np.random.seed(0)
        l, a = 5, 4
        alleles = ['A', 'C', 'G', 'T']
        X = np.array(list(generate_possible_sequences(l, alleles)))
    ...
    def test_full_kernel_alignment_variable_rho(self):
        np.random.seed(1)
