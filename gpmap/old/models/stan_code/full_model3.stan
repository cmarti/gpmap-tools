...
transformed parameters {
    ...
    activity = exp(-(dd .* dd) / scale + log_base_activity);
    ...
    for(c in 1:C){    
        for(i in 1:S)
            log_ki[c][,i] = -(mu[c] + X[c, i] * theta[c]');
        
        ki = exp(log_ki[c]);
        yhat[,c] = log(background + theta_0 * (ki * activity) ./ (1 + alpha * ki * ones));
    }
...
model {
    ...
    mu ~ normal(0, 5);                      // Prior on wt effect at the optimal distance
    log_base_activity ~ normal(0, 2);
    ...
    Lcorr_theta ~ lkj_corr_cholesky(0.5);                // Prior on corrolation of mutational effects
